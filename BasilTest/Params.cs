...
namespace org.herbal3d.BasilTest {
    public class Params : IParameters {
        ...
        public abstract class ParameterDefnBase {
            ...
            public Params context; // context for setting and getting values
            ...
            public override void SetValue(String valAsString) {
                ...
                if (parser != null) {
                    ...
                    try {
                        ...
                        value = setValue;
                        BasilTest.log.DebugFormat("{0} SetValue. {1} = {2}", _logHeader, name, setValue);
                    }
                    catch (Exception e) {
                        BasilTest.log.ErrorFormat("{0} Failed parsing parameter value '{1}': '{2}'", _logHeader, valAsString, e);
                    }
                }
                else {
                    ...
                    try {
                        ...
                        value = setValue;
                        BasilTest.log.DebugFormat("{0} SetValue. Converter. {1} = {2}", _logHeader, name, setValue);
                    }
                    catch (Exception e) {
                        BasilTest.log.ErrorFormat("{0} Conversion failed for {1}: {2}", _logHeader, this.name, e);
                    }
                }
            }
        ...
        public T P<T>(string paramName) {
            ...
            if (TryGetParameter(paramName, out ParameterDefnBase pbase)) {
                if (pbase is ParameterDefn<T> pdef) {
                    ...
                    BasilTest.log.ErrorFormat("{0} Fetched unknown parameter. Param={1}", _logHeader, paramName);
                }
            }
            return ret;
        }
